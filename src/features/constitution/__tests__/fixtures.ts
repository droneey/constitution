...
const createFakeFileTree = (files: Readonly<Files>): FileTree => ({
...
const textOf = (files: Readonly<Files>, path: string): string => {
  const text: string | undefined = files[path];

  if (text === undefined) {
    throw new Error(`the fixture has no file ${path}`);
  }

  return text;
};

const without = (files: Readonly<Files>, path: string): Files =>
  Object.fromEntries(
